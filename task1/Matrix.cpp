...
const Matrix operator+(const Matrix& m1, const Matrix& m2) // friend function은 맴버 함수가 아니다. Matrix 소속이 아닌거다. 
{
  if ((m1.dx != m2.dx) || (m2.dy != m2.dy)) return Matrix();
  Matrix temp(m1.dy, m1.dx); // Stack 할당
